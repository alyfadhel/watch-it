...
import 'package:watch_it/features/movies/presentation/widgets/see_all/build_see_all_upcoming.dart';

class SeeAllUpcomingScreen extends StatelessWidget {
  const SeeAllUpcomingScreen({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => sl<MoviesCubit>()..getUpcomingMovies(),
      child: BlocConsumer<MoviesCubit, MoviesState>(
        listener: (context, state) {},
        builder: (context, state) {
          var cubit = MoviesCubit.get(context);
          if(state is GetMoviesUpcomingLoadingState){
            return const Center(child: CircularProgressIndicator(),);
          }
          else if(state is GetMoviesUpcomingSuccessState){
            return Scaffold(
